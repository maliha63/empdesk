  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-3">
        <button onClick={() => setCurrent(new Date(year, month - 1, 1))} className="p-1 rounded-lg text-(--text-muted) hover:text-(--text-primary) hover:bg-(--bg-card2) transition-colors">
          <ChevronLeft size={14} />
        </button>
        <span className="text-sm font-medium text-(--text-primary)">{MONTHS[month]} {year}</span>
        <button onClick={() => setCurrent(new Date(year, month + 1, 1))} className="p-1 rounded-lg text-(--text-muted) hover:text-(--text-primary) hover:bg-(--bg-card2) transition-colors">
      ...
      <div className="grid grid-cols-7 mb-1">
        {DAYS.map((d) => (
          <div key={d} className={`text-[10px] font-medium text-center py-1 ${d === "Sa" || d === "Su" ? "text-red-400" : "text-(--text-muted)"}`}>{d}</div>
        ))}
      </div>
            ...
            className={`h-9 flex items-center justify-center text-[11px] font-medium transition-colors ${
              ...
              year === today.getFullYear() 
                ...
                : cell.type !== "curr" 
                  ...
                  : (i % 7 === 5 || i % 7 === 6) 
                    ? "text-red-400 hover:bg-(--bg-card2)" 
                    : "text-(--text-secondary) hover:bg-(--bg-card2) cursor-pointer"
            }`}
          >
            {cell.day}
          </div>
        ...
        {isManager ? (
          <>
            <div className="bg-white dark:bg-[#111827] border border-[#e2e8f0] dark:border-[#1f2a3d] rounded-2xl p-5 shadow-none hover:shadow-none">
              <div className="flex items-center justify-between mb-3"><p className="text-xs font-semibold text-(--text-muted) uppercase tracking-wide">Total Headcount</p><div className="w-8 h-8 rounded-lg bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center text-blue-600"><Users size={18} /></div></div>
              <p className="text-3xl font-bold text-(--text-primary)">{employees.length}</p>
            </div>
            <div className="bg-white dark:bg-[#111827] border border-[#e2e8f0] dark:border-[#1f2a3d] rounded-2xl p-5 shadow-none hover:shadow-none">
              <div className="flex items-center justify-between mb-3"><p className="text-xs font-semibold text-(--text-muted) uppercase tracking-wide">Action Items</p><div className="w-8 h-8 rounded-lg bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center text-amber-600"><Clock size={18} /></div></div>
              <p className="text-3xl font-bold text-amber-500">{pendingLeaves}</p>
            </div>
            <div className="bg-white dark:bg-[#111827] border border-[#e2e8f0] dark:border-[#1f2a3d] rounded-2xl p-5 shadow-none hover:shadow-none">
              <div className="flex items-center justify-between mb-3"><p className="text-xs font-semibold text-(--text-muted) uppercase tracking-wide">Global Attendance</p><div className="w-8 h-8 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center text-emerald-600"><CalendarCheck size={18} /></div></div>
              <p className="text-3xl font-bold text-emerald-600">87%</p>
            </div>
            <div className="bg-white dark:bg-[#111827] border border-[#e2e8f0] dark:border-[#1f2a3d] rounded-2xl p-5 shadow-none hover:shadow-none">
              <div className="flex items-center justify-between mb-3"><p className="text-xs font-semibold text-(--text-muted) uppercase tracking-wide">Active Divisions</p><div className="w-8 h-8 rounded-lg bg-purple-50 dark:bg-purple-500/10 flex items-center justify-center text-purple-600"><Building2 size={18} /></div></div>
              <p className="text-3xl font-bold text-(--text-primary)">{deptStats.length}</p>
            </div>
          </>
        ) : (
          <>
            <div className="bg-white dark:bg-[#111827] border border-[#e2e8f0] dark:border-[#1f2a3d] rounded-2xl p-5 shadow-none hover:shadow-none">
              <div className="flex items-center justify-between mb-3"><p className="text-xs font-semibold text-(--text-muted) uppercase tracking-wide">My Attendance</p><div className="w-8 h-8 rounded-lg bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center text-blue-600"><UserCheck size={18} /></div></div>
              <p className="text-3xl font-bold text-blue-600">96%</p>
            </div>
            <div className="bg-white dark:bg-[#111827] border border-[#e2e8f0] dark:border-[#1f2a3d] rounded-2xl p-5 shadow-none hover:shadow-none">
              <div className="flex items-center justify-between mb-3"><p className="text-xs font-semibold text-(--text-muted) uppercase tracking-wide">Leave Bank</p><div className="w-8 h-8 rounded-lg bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center text-amber-600"><Clock size={18} /></div></div>
              <p className="text-3xl font-bold text-(--text-primary)">{annualLeaveRemaining + medicalLeaveRemaining} <span className="text-xs text-(--text-muted) font-normal">Days Left</span></p>
            </div>
            <div className="bg-white dark:bg-[#111827] border border-[#e2e8f0] dark:border-[#1f2a3d] rounded-2xl p-5 shadow-none hover:shadow-none">
              <div className="flex items-center justify-between mb-3"><p className="text-xs font-semibold text-(--text-muted) uppercase tracking-wide">Tasks Finalized</p><div className="w-8 h-8 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center text-emerald-600"><CheckSquare size={18} /></div></div>
              <p className="text-3xl font-bold text-emerald-600">24</p>
            </div>
            <div className="bg-white dark:bg-[#111827] border border-[#e2e8f0] dark:border-[#1f2a3d] rounded-2xl p-5 shadow-none hover:shadow-none">
              <div className="flex items-center justify-between mb-3"><p className="text-xs font-semibold text-(--text-muted) uppercase tracking-wide">Active Assignment</p><div className="w-8 h-8 rounded-lg bg-purple-50 dark:bg-purple-500/10 flex items-center justify-center text-purple-600"><Briefcase size={18} /></div></div>
      ...
      {isManager ? (
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <div className="xl:col-span-2">
            <Card title="Corporate Workforce Trends" subtitle="Company Attendance Fluctuations vs Measured Productivity Performance" className="shadow-none hover:shadow-none">
          ...
          <div className="xl:col-span-1">
            <Card title="Division Headcounts" subtitle="Headcount breakdown metrics across primary departments" className="shadow-none hover:shadow-none">
              <ResponsiveContainer width="100%" height={260}>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <div className="xl:col-span-2">
            <Card title="My Engagement Ledger" subtitle="Personal Attendance Cycles vs Assignment Targets Completed" className="shadow-none hover:shadow-none h-full">
          ...
          <div className="xl:col-span-1">
            <Card title="Leave Allocation Balance" subtitle="Structural breakdown of available leave time" className="shadow-none hover:shadow-none h-full">
              <div className="flex flex-col items-center justify-center h-65 pt-2">
                ...
                <div className="flex flex-wrap justify-center gap-x-4 gap-y-1 mt-4 text-[11px] text-(--text-secondary) font-medium">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        <div className="lg:col-span-3 flex flex-col gap-6">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 items-stretch">
            ...
            <Card title="Notice board" subtitle="Global organizational logs" className="shadow-none hover:shadow-none">
              <div className="space-y-0 h-64 overflow-y-auto pr-1">
                {notices.slice(0, 4).map((notice, idx) => (
                  <div key={notice.id} className="flex gap-2.5 py-3 border-b border-(--border) last:border-0">
                    <div className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 ${notice.priority === "high" ? "bg-red-500" : "bg-blue-500"}`} />
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-semibold text-(--text-primary) truncate">{notice.title}</p>
            ...
            {isManager ? (
              <Card title="Pending requests" subtitle="Action items required" className="shadow-none hover:shadow-none">
                <div className="space-y-0 h-64 overflow-y-auto pr-1">
                  {requests.filter((r) => r.status === "pending").length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-(--text-muted) text-center py-12">
                      <CheckCircle2 size={32} className="text-emerald-500 mb-3" />
                      <p className="font-medium text-(--text-primary)">No pending leave requests</p>
                      <p className="text-sm mt-1">All requests have been processed.</p>
                    </div>
                  ) : (
                    <AnimatePresence>
                      {requests.filter((r) => r.status === "pending").map((r) => (
                        <motion.div key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-2 py-2.5 border-b border-(--border) last:border-0">
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-semibold text-(--text-primary) truncate">{r.name}</p>
              ...
              <Card title="My Work Assignments" subtitle="Active operation items tracker" icon={<ListTodo size={16} />} className="shadow-none hover:shadow-none">
                <div className="space-y-0 h-64 overflow-y-auto pr-1">
                  {PERSONAL_TASKS.map((task) => (
                    <div key={task.id} className="py-2.5 border-b border-(--border) last:border-0 flex flex-col gap-1">
                      <div className="flex justify-between items-start gap-2">
                        <p className="text-xs font-semibold text-(--text-primary) leading-tight">{task.title}</p>
                      ...
                      <div className="flex justify-between items-center mt-1">
                        <span className="text-[10px] text-(--text-muted)">Sprint Item</span>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white dark:bg-[#111827] border border-[#e2e8f0] dark:border-[#1f2a3d] rounded-2xl p-4 shadow-none hover:shadow-none h-full flex flex-col">
            ...
            <div className="pt-3 border-t border-(--border) mt-auto">
              {isManager ? (
                <div>
                  <p className="text-[10px] font-semibold text-(--text-muted) uppercase tracking-wide mb-1.5">Management Summary</p>
                  <div className="flex justify-between items-center bg-gray-50 dark:bg-gray-900/40 p-2 rounded-xl border border-gray-100 dark:border-gray-800">
                    ...
                    <span className="text-xs font-bold text-(--text-primary)">{pendingLeaves > 0 ? `${pendingLeaves} Tasks` : "Stable"}</span>
                  </div>
                </div>
              ) : (
