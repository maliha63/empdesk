...
export default function EmployeesPage() {
  ...
  const filtered = useMemo(() => {
    return employees
      ...
      .sort((a, b) => {
        const valA = sortBy === "name" ? `${a.firstName} ${a.lastName}` : a.company?.department ?? "";
        const valB = sortBy === "name" ? `${b.firstName} ${b.lastName}` : b.company?.department ?? "";
        return sortDir === "asc" ? valA.localeCompare(valB) : valB.localeCompare(valA);
      });
  }, [employees, debouncedSearch, dept, sortBy, sortDir]);

  const { page, setPage, totalPages, paginated, reset } = usePagination(filtered, ITEMS_PER_PAGE);

  const handleSearch = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    setSearch(e.target.value); reset();
  }, [reset]);

  function toggleSort(field: "name" | "dept") {
    ...
    <>
      ...
      <div className="space-y-5">
        <PageHeader
          ...
          crumbs={[{ label: "Dashboard", to: "/dashboard" }, { label: "Employees" }]}
          action={
            user?.role === "manager" ? (
              <button
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Dropdown
            options={deptOptions}
            value={dept}
            onChange={(val) => { setDept(val); reset(); }}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-surface-border">
                ...
                >
                  <span className="flex items-center gap-1">Department <SortIcon field="dept" /></span>
                </th>
                <th className="text-left px-4 py-3 text-xs font-medium text-slate-400 hidden lg:table-cell">Email</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-slate-400 hidden lg:table-cell">Title</th>
                ...
                paginated.map((e, i) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <img src={e.image} alt={e.firstName} className="w-8 h-8 rounded-full object-cover" />
                    </td>
                    <td className="px-4 py-3 text-white font-medium">{e.firstName} {e.lastName}</td>
                    <td className="px-4 py-3"><Badge variant="blue">{e.company?.department}</Badge></td>
                    <td className="px-4 py-3 text-slate-400 hidden lg:table-cell">{e.email}</td>
                    <td className="px-4 py-3 text-slate-400 hidden lg:table-cell">{e.company?.title}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-3">
                        ...
                        {user?.role === "manager" && (
                          <>
                            <button onClick={() => navigate(`/employees/${e.id}/edit`)}
        ...
        <div className="md:hidden space-y-3">
          {paginated.length === 0 ? (
            <p className="text-center py-10 text-slate-500 text-sm">No employees found.</p>
          ) : (
            paginated.map((e, i) => (
              <MobileCard
                key={e.id}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1)
                ...
                .map((p, i) =>
                  p === "..." ? (
                    <span key={`e-${i}`} className="px-2 py-1.5 text-xs text-slate-500">...</span>
                  ) : (
                    <button
                      key={p}
...
function MobileCard({ employee: e, index, isManager, onView, onEdit, onDelete }: MobileCardProps) {
  return (
    <motion.div
    ...
    >
      <img src={e.image} alt={e.firstName} className="w-10 h-10 rounded-full object-cover shrink-0" />
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-1.5"><Badge variant="blue">{e.company?.department}</Badge></div>
      </div>
      <div className="flex flex-col items-end gap-2">
        <button onClick={onView} className="text-slate-400 hover:text-white transition-colors"><Eye size={14} /></button>
        {isManager && (
          <>
            <button onClick={onEdit} className="text-brand-500 hover:text-brand-600 transition-colors"><Pencil size={14} /></button>
            <button onClick={onDelete} className="text-red-400 hover:text-red-300 transition-colors"><Trash2 size={14} /></button>
          </>
        )}
      </div>
    </motion.div>
  ...
  return (
    <div className="space-y-5 animate-pulse">
      ...
      <div className="bg-surface-card border border-surface-border rounded-xl overflow-hidden">
        {[...Array(8)].map((_, i) => (
          <div key={i} className="h-14 border-b border-surface-border px-4 flex items-center gap-3">
