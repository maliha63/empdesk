  ...
  return (
    <>
      ...
      <div className="space-y-6">
        <PageHeader
          title="Designation"
          description="Job roles and designations"
          crumbs={[{ label: "Dashboard", to: "/dashboard" }, { label: "Designation" }]}
          action={<Button onClick={() => setShowModal(true)}>+ Add Designation</Button>}
        />
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y">
              {designations.map((des, i) => (
                <tr key={des.id} className="hover:bg-gray-50 dark:hover:bg-[#0f172a]">
                  <td className="pl-8 py-5">{String(i+1).padStart(2,'0')}</td>
                  <td className="py-5 font-medium">{des.name}</td>
                  <td className="py-5 text-(--text-secondary)">{des.description}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-[#111827] rounded-2xl w-full max-w-md p-8">
            <h3 className="text-xl font-semibold mb-6">Add Designation</h3>
            <input type="text" placeholder="Designation Name" className="w-full border rounded-xl px-4 py-3 mb-4" value={form.name} onChange={e => setForm({...form, name: e.target.value})} />
